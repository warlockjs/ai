...
- Suite: **176 files / 1926 tests passing**, including the regression tests for each of the security fixes above (24 for the supervisor fan-out cap and the state-merge guard alone; 17 more for the `asTool` session binding and the memory scope isolation — session-A-cannot-hijack-session-B via the tool payload, per-tier cross-scope recall for all four tiers, and the explicit shared/tenant opt-ins)
