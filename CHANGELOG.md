...
- Suite: **176 files / 1943 tests, 1940 passing**, including the regression tests for each of the security fixes above (24 for the supervisor fan-out cap and the state-merge guard alone; 17 for the `asTool` session binding and the memory scope isolation — session-A-cannot-hijack-session-B via the tool payload, per-tier cross-scope recall for all four tiers, and the explicit shared/tenant opt-ins; 17 more for the working-memory cap, the `semanticCache` session scope, and the planner's parse-time step ceiling). The 3 failures are pre-existing and environmental — cold-`import("../ai")` barrel loads in `prompts-manager*.spec.ts` / `agent-identity.spec.ts` exceeding the 30s per-test timeout on a cold transform cache; they reproduce unchanged on the commit before these fixes and pass when run warm
